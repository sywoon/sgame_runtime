...
encode(const struct sproto_arg *args) {
	...
	case SPROTO_TINTEGER: {
		...
		vh = (int64_t)v >> 31;
		if (vh == 0 || vh == -1) {
			*(uint32_t *)args->value = (int32_t)v;
			return 4;
		}
		else {
			*(uint64_t *)args->value = (int64_t)v;
			return 8;
		}
	}
...
decode(const struct sproto_arg *args) {
	...
	case SPROTO_TINTEGER: {
		...
		if (args->extra) {
			...
			int64_t v = *(int64_t*)args->value;
			if (v > 2147483647 || v < -2147483648)  //[-2147483648,2147483647]   2147483647=(0x7fffffff- 1) == (2^31-1)
			{
				int64_t v = *(int64_t*)args->value;
				lua_Number vn = (lua_Number)v;
				lua_pushnumber(L, vn);
...
luaopen_sproto_core(lua_State *L) {
	...
	pushfunction_withbuffer(L, "encode_int32", lencode_int32);
	pushfunction_withbuffer(L, "decode_int8", ldecode_int8);
	pushfunction_withbuffer(L, "decode_int16", ldecode_int16);
	pushfunction_withbuffer(L, "decode_int32", ldecode_int32);
